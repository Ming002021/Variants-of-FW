...
module MyFW
    ...
    function Classical_FW_QP(epsil::Float64,K::Int64,Instance)
        ...
        ######################################################################################################
        x_1= KKT_StartPoint(Instance)#solveLP_StartPoint(Instance)  #StartPoint_Two_PhaseSimplex(Instance)      #solveLP_StartPoint(Instance)  #Get the starting point using solveLP_StartPoint or StartPoint_Two_PhaseSimplex
    ...
    function Pairwise_FW_QP(epsil::Float64,K::Int64,Instance)
        ...
        x_1= KKT_StartPoint(Instance) #KKT_StartPoint(Instance) #solveLP_StartPoint(Instance)#KKT_StartPoint(Instance)    ##      #solveLP_StartPoint(Instance)  #Get the starting point using solveLP_StartPoint or StartPoint_Two_PhaseSimplex
        global x_k=x_1                                         #Plug the initial point
        global iter=0                                          #Initialize the iteration number to be 0
        global rep=1                                           #Binary variabe to control if the while loop should be discountinued
        global opt_val=Inf                                     #Initialize the final objective function value to be 0
    ...
    function AwayStep_Atoms_FW_QP(epsil::Float64,K::Int64,Instance)
        ...
        x_1= KKT_StartPoint(Instance)#solveLP_StartPoint(Instance) #KKT_StartPoint(Instance) #solveLP_StartPoint(Instance)  #KKT_StartPoint(Instance)    #StartPoint_Two_PhaseSimplex(Instance)  #      #Get the starting point using solveLP_StartPoint or StartPoint_Two_PhaseSimplex
        global x_k=x_1                                         #Plug the initial point
        global iter=0                                          #Initialize the iteration number to be 0
        global rep=1                                           #Binary variabe to control if the while loop should be discountinued
        global opt_val=Inf                                     #Initialize the final objective function value to be 0
        ...
        while rep==1
            ...
            if gradient(x_k,Instance)'*d_Tk <= gradient(x_k,Instance)'*d_Ak #The Toward direction makes the LP reduced more
                ...
                if z_k in D == true                      #If z_k already in D
                    ...
                    Weight_D[Index_zk_D] .= (1-gamma_k) * Weight_D[Index_zk_D] .+gamma_k #Update weights of all atoms in D 
                    if isempty(Non_Index_zk_D) ==false

                        Weight_D[Non_Index_zk_D] .= (1-gamma_k) * Weight_D[Non_Index_zk_D]
                    end
    ...
    function  Pairwise_Atoms_FW_QP(epsil::Float64,K::Int64,Instance)
        ...
        while rep==1
            ...
            if gamma_k!=0  #x_k=x_k+gamma_k*d_k=x_k+gamma_k*(z_k-w_k)
                ...
                Weight_D[Index_wk_D] =Weight_D[Index_wk_D] .-gamma_k
    
                if z_k in D == true                                           #If z_k is already in D
                    
                    Index_zk_D=findall(x-> x==z_k,D)                          #Get the index of w_k in D
                    Weight_D[Index_zk_D] .= Weight_D[Index_zk_D].+gamma_k     #Update the weight of w_k 
