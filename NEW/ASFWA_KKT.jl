###############################################################################
# Away-steps Frank-Wolfe Algorithm with active atoms (in atomic version)        #
...
for j in 1:epsil_length
    ...
    for i in 1:instance_Type_length     
        ...
        output2_path="/Users/Mindy/Desktop/NEW/Text/ASFWA_KKT/"                   #Go to the directory used to store output text files                          
        cd(output2_path)
        ...
        open("$(mat_file)_$(epsilName).txt", "w") do file  
            ...
            write(file,"The  Away-Steps Frank-Wolfe Algorithm in Atomic Version with KKT is Starting:\n") 
            write(file, "\n")
