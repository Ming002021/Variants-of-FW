#################################################################################
#Use BenchmarkTools to get the running time of Pairwise  FW in atomic version   #
...
for j in 1:epsil_length
    ...
    for i in 1:instance_Type_length     
        ...
        b = @benchmark MyFW.PFWA($x_1,$K,$epsil,$A,$Aeq,$b,$beq,$LB,$UB,$Q,$c,$T,$n) evals=10 samples=100 seconds = 10000;
        min = minimum(b)
        runtimes=min.time/1e9
        println("Running Time is $(runtimes)")
        
        output2_path="/Users/Mindy/Desktop/NEW_RT/Text/PFWA_Phase I/$(instance_Type)"                                
        cd(output2_path)
